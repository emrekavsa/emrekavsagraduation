...
export default function Home() {
  ...
  const fetchPolls = useCallback(
    async (isInitial = false) => {
      ...
      let query = supabase
        .from("polls")
        .select("id, title, created_at, poll_options(id, content, image_url)")
  ...
  const handleVote = async (pollId, optionId) => {
    ...
    if (error) {
      alert(error.code === "23505" ? "Already voted!" : "Error occurred");
    } else {
    ...
    >
      ...
      <div className="max-w-xl mx-auto p-4 mt-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Vote on community topics</p>
        </div>
        ...
        {dataLoading && (
          <div className="text-center py-6 font-medium text-blue-600">
            Loading more...
          </div>
        )}
      </div>
